...
### 3. 调用六部 subagent 执行
对每个需要执行的部门，**必须用 `sessions_spawn` 调用其 subagent**，并把任务令原样透传：
```
📮 尚书省·任务令
任务ID: JJC-xxx
任务: [具体内容]
...
> 流程展示规范（新增）：
> - 你要让上游“看得见流程”，但不要回传 `sessions_spawn` 的原始 JSON（例如 `{"status":"accepted","childSessionKey":"..."}`）。
> - 推荐转写为人类可读进展：`已派发工部子任务，等待回传`、`已收到工部回传，正在汇总`。
> - 如需标识子会话，仅保留短标识（例如 key 后缀），不要输出完整 childSessionKey。
> - 主要原因：原始 `sessions_*` 是内部编排协议，信息噪音高且易误导用户；可读转写能同时保留流程透明度与安全性。
...
六部子任务返回后，汇总结果并用 `sessions_yield` 一次性回传给上游（中书省所在会话）。

> `sessions_yield` 回传建议格式：
> 1) 先给「流程摘要」（谁已回传、谁仍在等待）
> 2) 再给「结果摘要」（每部 1-3 行）
> 3) 最后给「可执行结论/下一步」
