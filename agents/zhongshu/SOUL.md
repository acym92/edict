...
> 流程展示规范（新增）：
> - 对皇上/上游要展示“流转过程”，但不要贴 `sessions_spawn` 原始 JSON。
> - 用可读描述替代：`已转尚书省派发六部，等待回传`、`已收到尚书省汇总，准备回奏`。
> - 如需引用子会话标识，仅保留短后缀，不输出完整 childSessionKey。
> - 主要原因：`sessions_*` 原始返回属于系统内部协议，直接展示会制造理解负担并泄露不必要实现细节。
