...
## ❓ 常见疑问：`sessions_yield` 是报错吗？

很多同学会在 OpenClaw/会话日志里看到类似：

```json
{
  "status": "yielded",
  "message": "…已从 Zhongshu 流转至 Menxia，等待门下省审议结果…"
}
```

这通常**不是异常报错**，而是「当前 Agent 主动让出会话控制权（yield）」的状态信号，语义接近：

- 当前步骤已经执行完（例如中书省已把任务提交给门下省）
- 下一步需要等待下游 Agent（如门下省）返回结果
- 调度器会在后续通过派发、重试、升级唤醒继续推进

### 为什么会看到它？

1. **中书省阶段性完成后等待门下省**  
   典型路径是 `Zhongshu -> Menxia`，处于审议等待态。  
2. **任务没有终结，而是流转中**  
   任务状态不是 `Done/Cancelled`，而是 `Menxia/Assigned/Doing` 等中间态。  
3. **调度层在做异步推进**  
   太子调度器会在停滞阈值后触发重试、升级唤醒（门下省/尚书省）或回滚。  

### 什么时候才算“真的卡住”？

可按下面顺序判断：

1. 看 `state` 是否长期不变且无新增 `flow_log/progress_log`
2. 看 Agent 面板是否变为 `⚪ 休眠` 且长时间无 `lastActive` 更新
3. 执行或调用调度扫描（scheduler scan）后仍无动作记录

如果上述都满足，再按「重试 → 升级 → 回滚」流程介入处理。
...
curl -X POST http://0.0.0.0:7891/api/create-task \
...
curl http://0.0.0.0:7891/api/task-activity/JJC-20260302-001
